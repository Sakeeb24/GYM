...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      // Authenticated: redirect away from auth public routes.
      if (isPublic && currentPath != '/display/attendance') return '/app';
      return null;
    },
    routes: [
      GoRoute(
      ...
      // ── Dedicated Fullscreen Gym Kiosk Attendance Display Mode ─────────
      GoRoute(
        path: '/display/attendance',
        name: 'display-attendance',
        builder: (context, state) {
          final slug = state.uri.queryParameters['gym'];
          return GymDisplayScreen(gymSlug: slug);
        },
      ),
