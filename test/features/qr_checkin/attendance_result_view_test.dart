...
void main() {
  ...
  testWidgets('AttendanceResultView renders custom message when supplied', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: AttendanceResultView(
            outcome: CheckInOutcome.denied,
            message: 'Check-in denied: membership is expired. Please renew.',
          ),
        ),
      ),
    );
    ...
    expect(find.text('ACCESS DENIED'), findsOneWidget);
    expect(find.text('Check-in denied: membership is expired. Please renew.'), findsOneWidget);
  });
}
