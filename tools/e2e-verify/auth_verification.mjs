...
// Comprehensive End-to-End Verification Suite for LiftFlow (Owner QR Member Activation)
...
async function run() {
  ...
  // TEST 1: RegisterMember without valid activation token is rejected
  console.log('1. Testing registerMember with test phone 7019707247 & invalid activation token...');
  const regRes = await testEndpoint(
    'registerMember (Invalid Activation Token)',
    `${PROJECT_URL}/functions/v1/registerMember`,
    {
      ...
      body: JSON.stringify({
        ...
        phone: '7019707247',
        activation_token: 'invalid_unauthorized_token_hex_999999',
  ...
  const wrongTokenRejected = regRes.status === 400 || regRes.status === 404 || regRes.status === 410 || regRes.status === 401;
  console.log(`   Result: HTTP ${regRes.status} -> ${regRes.bodyText.slice(0, 100)}`);
  report.push({
    category: 'Authentication - Forged / Invalid QR Token Rejection',
    status: wrongTokenRejected ? 'PASS' : 'FAIL',
    details: `HTTP ${regRes.status}: Invalid activation token rejected as expected.`
  });

  // TEST 2: validateMemberActivation with forged token is rejected
  console.log('\n2. Testing validateMemberActivation with forged token...');
  const valRes = await testEndpoint(
    'validateMemberActivation (Forged Token)',
    `${PROJECT_URL}/functions/v1/validateMemberActivation`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        token: 'forged_fake_activation_token_1234567890',
      }),
    }
  );
  const valBlocked = valRes.status === 404 || valRes.status === 400;
  console.log(`   Result: HTTP ${valRes.status} -> ${valRes.bodyText.slice(0, 100)}`);
  report.push({
    category: 'Authentication - validateMemberActivation Guard',
    status: valBlocked ? 'PASS' : 'FAIL',
    details: `HTTP ${valRes.status}: Forged activation token rejected.`
  });

  // TEST 3: Duplicate Phone / Account Takeover Prevention Check
  console.log('\n3. Testing duplicate phone protection...');
  const dupCheck = await testEndpoint(
    ...
    `${PROJECT_URL}/functions/v1/registerMember`,
    {
      ...
      body: JSON.stringify({
        ...
        activation_token: 'fake_activation_token_12345',
  ...
  // TEST 4: Password Recovery Endpoint
  ...
  // TEST 5: QR Check-in Endpoint Security (Forged / Unsigned Token Rejection)
  ...
  // TEST 6: Payment Webhook Signature Guard
  ...
  // TEST 7: Protected Tables RLS
  console.log('\n7. Testing RLS isolation across tables with Anon Key...');
  const tables = ['members', 'gym_settings', 'audit_logs', 'payments', 'member_activation_tokens'];
